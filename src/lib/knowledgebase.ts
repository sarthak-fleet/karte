...
function ragServiceKey(): string {
  return (
    process.env.RAG_SERVICE_KEY?.trim() ||
    cloudflareEnv().RAG_SERVICE_KEY?.trim() ||
    ''
  );
}
...
export async function createIndex(name: string): Promise<{ id: string }> {
  ...
  if (!res.ok)
    throw new Error(`Failed to create RAG index: ${await res.text()}`);
  return res.json();
}
...
export async function ingestDocument(
  ...
  if (!res.ok)
    throw new Error(`Failed to ingest RAG document: ${await res.text()}`);

  const data = (await res.json()) as {
    documents?: Array<{ document_id: string; chunks_created: number }>;
  };
...
export async function deleteDocument(docId: string): Promise<void> {
  ...
  if (!res.ok && res.status !== 404)
    throw new Error(`Failed to delete RAG document: ${await res.text()}`);
}
