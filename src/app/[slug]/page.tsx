...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // og:image is generated by /api/og/[slug] — but the OG route runs
  // in a context where the D1 binding hangs, so we bake every datum
  // the renderer needs into the URL query: live newspaper headline
  // (or bio fallback), display name, location, theme accent colors.
  // Cache hit at the edge for the same params; new params on save.
  ...
  const liveHeadline =
    typeof newsHeadline === 'string' && newsHeadline.trim()
      ...
      : null;
  const ogHeadline =
    liveHeadline ?? page.bio ?? `Visit ${page.displayName} on Karte`;
  const ogParams = new URLSearchParams({
    name: page.displayName,
    h: ogHeadline.slice(0, 200),
    loc: page.location ?? '',
    accent: theme.accentColor.replace('#', ''),
    grad2: (theme.gradientTo ?? theme.accentColor).replace('#', ''),
    live: liveHeadline ? '1' : '0',
  });
  const ogUrl = `/api/og/${slug}?${ogParams.toString()}`;
